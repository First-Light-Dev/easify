 ...
 * ceiling. Resource classes build documents on top of it.
 */
export class ShipHeroClient {
  ...
  async request<TData = unknown>(
    ...
    const operation =
      options.operationName ?? document.match(/^\s*(?:query|mutation)\s+(\w+)/)?.[1] ?? 'anonymous';
    let refreshed = false;
  ...
  query<TData = unknown>(
    ...
    return this.request<TData>(document, variables, options);
  }
